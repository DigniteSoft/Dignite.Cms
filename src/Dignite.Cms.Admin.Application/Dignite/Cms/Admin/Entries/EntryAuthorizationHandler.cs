...
namespace Dignite.Cms.Admin.Entries
{
    public class EntryAuthorizationHandler : AuthorizationHandler<OperationAuthorizationRequirement, Entry>
    {
        ...
        {
            if (await _permissionChecker.IsGrantedAsync(context.User, Permissions.CmsPermissions.Entry.Delete))
            {
                if (await _permissionChecker.IsGrantedAsync(context.User, Permissions.CmsPermissions.Page.SuperAuthorization))
                    return true;
                else
                {
        ...
        {
            if (await _permissionChecker.IsGrantedAsync(context.User, Permissions.CmsPermissions.Entry.Update))
            {
                if (await _permissionChecker.IsGrantedAsync(context.User, Permissions.CmsPermissions.Page.SuperAuthorization))
                    return true;
                else
        ...
        {
            if (await _permissionChecker.IsGrantedAsync(context.User, Permissions.CmsPermissions.Entry.Create))
            {
                if (await _permissionChecker.IsGrantedAsync(context.User, Permissions.CmsPermissions.Page.SuperAuthorization))
                    return true;
                else
